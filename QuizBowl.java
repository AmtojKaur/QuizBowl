   ...
   */
   public static void fillQuestionBank(Scanner fileIp, ArrayList<Question> qBank)  {
      ...
      while(fileIp.hasNextLine()) {
         ...
         String qType = fileIp.next();
         int point = fileIp.nextInt();
         fileIp.nextLine();
         String question = fileIp.nextLine();
         
         if(qType.equals("TF")) {
         
            String rightAns = fileIp.next();
            qBank.add(new QuestionTF(question, point, rightAns));
            
         } else if (qType.equals("SA")) {
            
            String rightAns = fileIp.next();
            qBank.add(new QuestionSA(question, point, rightAns));
            
         } else {
            
            int choicesNum = fileIp.nextInt();
            fileIp.nextLine();
            
            ArrayList<String> choiceList = new ArrayList<String>(choicesNum);
            
            for(int i = 1; i <= choicesNum; i++) {
               choiceList.add(fileIp.nextLine());
            }
            String rightAns = fileIp.next();
            qBank.add(new QuestionMC(question, point, rightAns, choiceList));
         }  
      }
      fileIp.close();
   }
